...
function get_qr_code_url($username, $secret) {
	$h = 200;
	return  'http://chart.apis.google.com/chart?chs=' . $h . 'x' . $h . '&chld=M|0&cht=qr&chl=' . urlencode( 'otpauth://totp/' . $username . '?secret=' . $secret );
}

function get_qr_code($username, $secret) {
	if(FALSE === $secret) {
		return FALSE;
	}
	$url = get_qr_code_url($username, $secret);
